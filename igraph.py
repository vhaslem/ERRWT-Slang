...
# Valon
# 11-13-2025
# This code pulls from the training dataset and uses igraph to graph instances
# just uses english/nonenglish packages, not errwt
#############
...
def tokenize_texts_iter(csv_path, chunksize=CHUNKSIZE):
...
total_docs = 0
for tokens in tqdm(tokenize_texts_iter(CSV_PATH), desc='Pass1 token counting'):
...
for tokens in tqdm(tokenize_texts_iter(CSV_PATH), desc='Pass2 building cooc'):
...
# We'll check wordnet membership only for the top tokens (cheap)
import nltk
...
english_flags = {w: is_english_word(w) for w in vocab}
# Build sets
eng_words = {w for w, flag in english_flags.items() if flag}
slang_candidates = {w for w, flag in english_flags.items() if not flag}
...
with open('output1/word_weights_top_vocab_large.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
...
slang_indices = [vocab_index[w] for w in slang_candidates]
slang_sorted = sorted(slang_indices, key=lambda i: row_sums[i], reverse=True)
with open('output1/slang_weights_top_vocab_large.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
...
# Save graph
nx.write_gml(G, 'output1/slang_network_top.gml')
...
nx.draw(G, pos, with_labels=False, node_size=50, width=np.log1p(weights), edge_cmap='viridis')
plt.savefig('output1/slang_graph_top.png', dpi=300)
